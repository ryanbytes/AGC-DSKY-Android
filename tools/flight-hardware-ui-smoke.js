...
for(const marker of ["const KEY_CONTACT_BASE_MS = 36","const KEY_RETURN_SOUND_BASE_MS = 18","const HARDWARE_SEED_KEY = 'dskyHardwareUnitSeedV1'","contactMs: Number(vary(KEY_CONTACT_BASE_MS","returnSoundMs: Number(vary(KEY_RETURN_SOUND_BASE_MS","makePitch: Number(vary(520","returnPitch: Number(vary(330","--key-travel","function prepareKeys()","const lighting = Object.freeze({","function hardwarePersonality()", "window.AGCDSKY_SERVICE_REGISTRY.publish('AGCDSKY_FLIGHT_HARDWARE_UI',Object.freeze({lighting,hardwarePersonality})"])req(ui,marker,'key presentation personality service');
...
for(const forbidden of ["vary(KEY_CONTACT_BASE_MS",'window.AGCDSKY.hardwarePersonality =','window.AGCDSKY.keyMechanicalSpec =','window.AGCDSKY_FLIGHT_HARDWARE_UI'])no(keySpec,forbidden,'key mechanics facade/compatibility/timing ownership');
...
for(const marker of ['--key-el-color','--key-el-filter','.key.pressed','translateY(var(--key-travel,.42vmin))','fill:var(--key-el-color','filter:var(--key-el-filter'])req(finish+controls,marker,'white EL key illumination / vector styling');
