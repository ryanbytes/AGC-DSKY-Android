...
// Normal-key electrical interlock remains the sole channel-015 owner and uses
// the centralized input runtime for make/KEYRST rather than touching core I/O.
for(const marker of ["window.addEventListener('pointerdown', onPointerDown, {capture:true, passive:false})","if (!button || button.dataset.key === 'P') return null","const accepted = !cycleLatched","if (!state.accepted) return","if (!allNormalKeysReleased()) return","const MIN_KEYCODE_HOLD_MS = 12","const remaining = MIN_KEYCODE_HOLD_MS - elapsed","keyResetPending:!!keyResetTimer","const input = api?.inputRuntime","input.keyMake(code)","input.keyReset(electricalCore)","electricalKeyCode","keyboardElectrical"])req(interlock,marker,'series-contact keyboard interlock');
for(const forbidden of ["P:0o",".keyPress(",".keyRelease(","writeIo(0o15"])no(interlock,forbidden,'series-contact keyboard direct core ownership');
