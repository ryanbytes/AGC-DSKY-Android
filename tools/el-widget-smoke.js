...
req(layout,'<TextClock','widget layout');
no(layout,'<ViewFlipper','widget layout');
no(layout,'<AdapterViewFlipper','widget layout');
req(layout,'android:id="@+id/el_hour_clock"','hour clock');
req(layout,'android:id="@+id/el_minute_clock"','minute clock');
req(layout,'android:id="@+id/el_seconds_clock"','seconds clock');
req(layout,`android:format24Hour="'+000'HH"`,'hour clock format');
req(layout,`android:format24Hour="'+000'mm"`,'minute clock format');
req(layout,`android:format24Hour="'+000'ss"`,'seconds clock format');
req(info,'android:updatePeriodMillis="1800000"','widget metadata');
req(info,'android:widgetCategory="home_screen"','widget metadata');
req(provider,'int[] clocks={R.id.el_hour_clock,R.id.el_minute_clock,R.id.el_seconds_clock}','platform clock overlays');
req(provider,'views.setTextViewTextSize(id,TypedValue.COMPLEX_UNIT_SP,18f*scaleDp)','clock text scaling');
no(provider,'populateFlipper(','legacy static flipper population');
no(provider,'setRemoteAdapter(','launcher remote-adapter dependency');
no(provider,'setExactAndAllowWhileIdle','alarm-driven clock tick');
...
req(layout,'android:layout_marginTop="92.526dp"','native clock hour datum');
