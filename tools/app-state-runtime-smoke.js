...
assert(context.readonlyProbe===42&&compat.get('readonlyProbe')===42&&readonlyProbe.version()===0,'compatibility read-only view bootstrap changed');
let readonlyRejected=false;try{compat.replace('readonlyProbe',7,'forbidden replacement')}catch(error){readonlyRejected=error instanceof TypeError}assert(readonlyRejected,'read-only compatibility view accepted explicit replacement');
assert(compat.describe().some(item=>item.name==='readonlyProbe'&&item.kind==='readonly'&&item.version===0),'read-only compatibility diagnostics changed');
...
assert(!source.includes('function mutable(')&&!source.includes('function accessor('),'compatibility registry retained mutable ownership primitives');
...
console.log('  sealed explicit app/core state, forwarding/read-only compatibility registry, owner-backed alias provenance, idempotent bootstrap, and explicit consumers verified');