...
assert(js.includes('async function acquireCamera(){'), 'camera acquire helper missing');
...
assert(js.includes("if (document.hidden || !view.classList.contains('open'))"), 'late getUserMedia result must be discarded when no longer visible/open');
