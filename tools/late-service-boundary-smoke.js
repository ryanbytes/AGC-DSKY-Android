...
for(const [name,source,markers] of [
 ...
 ['stretch',stretch,["compat.replace('set2'","compat.replace('setReg'","display.implementation('decodeChannel10')","display.installImplementation('decodeChannel10'",'visual.withSettledWordOverride']],
...
assert(!stretch.includes("compat.get('decodeChannel10'")&&!stretch.includes("compat.replace('decodeChannel10'"),'stretch stability channel wrapper must route through AGCDSKY_DISPLAY');
...
console.log('late service boundary smoke: PASS');console.log('  late geometry/hardware hooks use audited compat where still required; relay digit/channel visual/stretch/audio wrappers route through owning services; perceptual exaggeration stays opt-in; no bare slot, timer, or public-API monkey-patching remains');
