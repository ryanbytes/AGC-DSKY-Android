...
assert(css.includes('size:Letter portrait'), 'model checklist must print on US Letter paper');
assert(css.includes('grid-template-columns:repeat(2,minmax(0,1fr))!important'), 'model checklist must impose two cut-out pages across');
assert(css.includes('grid-auto-rows:5.16in!important'), 'model checklist must impose two cut-out rows per Letter sheet');
assert(css.includes('.cheat-pane:nth-child(4n+1):not(:first-child)'), 'model checklist must start a new sheet after four cut-out pages');
assert(css.includes('outline:.35pt dashed #999!important'), 'model checklist must include visible cut guides');
assert(activity.includes('PrintAttributes.MediaSize.NA_LETTER.asPortrait()'), 'legacy Android checklist must request Letter portrait media');
...
assert(sensorActivity.includes('PrintAttributes.MediaSize.NA_LETTER.asPortrait()'), 'real Android launcher checklist must request Letter portrait media');
...
assert(apple.includes('width: 8.5 * 72.0, height: 11.0 * 72.0'), 'Apple checklist print media must be US Letter');
