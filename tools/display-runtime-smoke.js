...
const before=renderer.compatibilityVersions().renderDigits;vm.runInContext("renderDigits=function(el,text){el.innerHTML='replacement:'+text}",context);renderer.set2('prog','88');assert(elements.prog.innerHTML==='replacement:88','renderer service did not dispatch through late geometry slot');assert(renderer.compatibilityVersions().renderDigits===before+1,'renderer replacement was not versioned through compatibility registry');
for(const token of ['const compat=window.AGCDSKY_COMPAT;','const SEG=','const PATH=','compat.mutable(\'renderDigits\'','compat.mutable(\'set2\'','compat.mutable(\'setLamp\'','window.AGCDSKY_RENDERER=Object.freeze({']){assert(source.includes(token),`renderer missing ${token}`)}
...
console.log('display runtime smoke: PASS');console.log('  EL primitives, frozen renderer service, and versioned late geometry replacement through the audited compatibility boundary verified');
