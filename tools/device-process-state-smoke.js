...
/*
 * Process-recreation state helper for AGC-DSKY Android.
 *
 * This helper is called on each side of an ADB force-stop/relaunch by
 * tools/device-process-recreation-smoke.sh. It stores the user's original
 * frontend preferences in temporary localStorage keys, puts the app into CM
 * AGC mode, then after process recreation verifies that the persisted CM/AGC
 * request created a fresh running yaAGC core. Finally it restores the original
 * frontend preferences and removes its temporary markers.
 *
...
if (!Number.isInteger(port) || port < 1 || port > 65535
    || !['prepare', 'verify', 'restore'].includes(action)) {
  console.error('usage: node tools/device-process-state-smoke.js <forwarded-devtools-port> <prepare|verify|restore>');
  process.exit(2);
}
...
async function findAppTarget() {
  ...
  if (!app) throw new Error('interactive AGC DSKY WebView target not found');
...
async function readMarkers(cdp) {
  return cdp.evaluate(`(() => ({
...
async function clearMarkers(cdp) {
  await cdp.evaluate(`(() => {
...
async function restoreFromMarkers(cdp, markers) {
  ...
  if (originalRunMode === 'agc') {
    ...
    await cdp.evaluate(`localStorage.setItem('runMode', ${JSON.stringify(originalRunMode)}); true`);
  }

  await clearMarkers(cdp);
  return true;
}

async function prepare(cdp) {
  const stale = await readMarkers(cdp);
  if (stale.nonce) {
    console.error('warning: restoring stale process-smoke markers before new prepare');
    await restoreFromMarkers(cdp, stale);
  }
  ...
  const state = await enterMission(cdp, 'comanche055');
  assert(state.storedMission === 'comanche055' && state.runMode === 'agc',
    `prepared state did not persist: ${JSON.stringify(state)}`);
  console.log('Process recreation prepare: PASS');
  console.log(`  nonce: ${nonce}`);
  console.log(`  CM core: ${state.version}`);
}

async function verify(cdp) {
  const markers = await readMarkers(cdp);
  assert(markers.nonce, 'process-smoke marker did not survive process recreation');
  let verified = false;
  try {
    const state = await poll(cdp, 'process recreation AGC restore', (s) =>
      s.ready
      && s.mission === 'comanche055'
      && s.storedMission === 'comanche055'
      && s.runMode === 'agc'
      && s.core
      && s.running
      && typeof s.version === 'string'
    ...
    try {
      ...
      console.error(`warning: could not restore pre-smoke frontend state: ${error.message}`);
...
async function main() {
  ...
  try {
    ...
    if (action === 'prepare') await prepare(cdp);
    else if (action === 'verify') await verify(cdp);
    else {
      const markers = await readMarkers(cdp);
      if (markers.nonce) {
        await restoreFromMarkers(cdp, markers);
        console.log('Process recreation state restore: PASS');
      } else {
        console.log('Process recreation state restore: nothing to restore');
      }
    }
  } finally {
    cdp.close();
...
main().catch((error) => {
  console.error(`DEVICE PROCESS STATE SMOKE FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
