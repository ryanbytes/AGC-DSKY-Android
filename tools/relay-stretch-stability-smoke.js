...
try{
  ...
  compat.readonly('SEG',()=>SEG);compat.mutable('relayDigit',code=>chars[Number(code)&31],isFn);compat.mutable('set2',(id,text)=>pairRenders.push({id,text:String(text)}),isFn);compat.mutable('setReg',(id,sign,digits)=>regRenders.push({id,sign:String(sign),digits:String(digits)}),isFn);compat.mutable('decodeChannel10',()=>{},isFn);
  context.AGCDSKY_DISPLAY={relayDigit:code=>compat.get('relayDigit')(code),status:()=>({display:agcDisplay,relayWords:{10:hardware.latches[10]}})};
  ...
  const set2=(id,text)=>compat.get('set2')(id,text),decode=value=>compat.get('decodeChannel10')(value),maskForChar=ch=>{let mask=0,order='abcdefg',segments=SEG[ch]||'';for(let i=0;i<order.length;i++)if(segments.includes(order[i]))mask|=1<<i;return mask};
  ...
  console.log('relay stretch stability smoke: PASS');console.log('  explicit compat wrappers preserve monotonic stretched segments, same-state DOM suppression, and authentic pass-through without global monkey-patching');
