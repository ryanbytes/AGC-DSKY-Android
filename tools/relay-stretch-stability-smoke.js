...
try{
  ...
  for(const token of ["display.implementation('decodeChannel10')","display.installImplementation('decodeChannel10'","renderer.implementation('set2')","renderer.installImplementation('set2'","renderer.implementation('setReg')","renderer.installImplementation('setReg'",'renderer.segmentPattern(ch)','renderer.registerSegmentPattern(ch,value)'])if(!source.includes(token))fail(`explicit service ownership missing: ${token}`);
  if(source.includes('AGCDSKY_COMPAT')||source.includes('compat.'))fail('stretch stability regained direct compatibility-registry dependency');
  ...
  console.log('relay stretch stability smoke: PASS');console.log('  renderer/display-owned wrappers preserve monotonic stretched segments, same-state DOM suppression, and authentic pass-through with no direct compat dependency');
