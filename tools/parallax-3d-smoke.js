...
for(const token of [
  'perspective:680px','rotateX(var(--dsky-tilt-x)) rotateY(var(--dsky-tilt-y))',
  '.ann-well','.display-well','.ann-grid','.elpanel','.el-glass-rear','.el-glass-sheen','.key.pressed',
  'translateY(var(--key-travel,.42vmin))','left:57.5000%','top:5.1075%','width:33.125%','height:49.0204%',
  'var(--dsky-el-z,-6.034px)','var(--dsky-glass-rear-z,-6.034px)','clip-path:polygon(',
  '@media (prefers-reduced-motion:reduce)','body.parallax-3d.screen-only:not(.dream):not(.display-only) .elpanel',
...
for(const mode of [':not(.dream)',':not(.display-only)']) assert(css.includes(mode),`parallax CSS does not exclude ${mode}`);

for(const token of [
  "matchMedia('(prefers-reduced-motion: reduce)')","matchMedia('(hover: hover) and (pointer: fine)')",
  "dsky.addEventListener('pointermove'","dsky.addEventListener('pointerdown'","window.addEventListener('deviceorientation'",
  "installNativeQuaternionTap()","api.nativePhoneQuaternion = wrapped","'native-quaternion'",
  "nativeActive:() => performance.now() - nativeSeenAt < NATIVE_PRIORITY_MS",
  "const GLASS_CLEAR_WIDTH_IN = 2.354","const GLASS_EDGE_THICKNESS_IN = 0.109","const GLASS_CENTER_RISE_IN = 0.025",
  "const GLASS_VIEW_THICKNESS_IN = GLASS_EDGE_THICKNESS_IN + GLASS_CENTER_RISE_IN","function updatePhysicalDepth()",
  "glassFront || elPanel","'--dsky-glass-rear-z'","'--dsky-el-z'","'--dsky-glass-depth-px'",
  "glassRear.className = 'el-glass-rear'","dsky.insertBefore(glassRear, glassFront)",
  "new ResizeObserver(updatePhysicalDepth).observe(glassFront)","geometry:() => Object.freeze({",
  "glassClearWidthIn:GLASS_CLEAR_WIDTH_IN","glassEdgeThicknessIn:GLASS_EDGE_THICKNESS_IN",
  "glassCenterRiseIn:GLASS_CENTER_RISE_IN","glassViewThicknessIn:GLASS_VIEW_THICKNESS_IN",
  "{passive:true}","body.classList.contains('dream')","body.classList.contains('display-only')",
  "glassFront.className = 'el-glass-sheen'",'window.AGCDSKY_PARALLAX = controller','api.parallax3d = controller',
  "const TILT_STORAGE_KEY = 'dskyParallaxTiltPct'","const DEPTH_STORAGE_KEY = 'dskyParallaxDepthPct'",
  'const MAX_INTENSITY_PERCENT = 200','function installIntensityControls()','parallax-tilt-intensity','parallax-depth-intensity',
  "input.type = 'range'","input.step = '5'",'const tiltScale = tiltPercent / 100','const depthScale = depthPercent / 100',
  'setTiltPercent:value => updateTiltPercent(value, true)','setDepthPercent:value => updateDepthPercent(value, true)',
  'installIntensityControls()'
]) assert(js.includes(token),`parallax controller missing ${token}`);

for(const token of [
  '.app-controls .parallax-controls','.app-controls .parallax-control','.app-controls .parallax-control span',
  '.app-controls .parallax-control input[type="range"]','width:126px','accent-color:#e8eee3'
...
for(const forbidden of [
  '--dsky-el-x','--dsky-el-y','--dsky-glass-x','--dsky-glass-y','--dsky-fs-el-x','--dsky-fs-el-y','--dsky-fs-glass-x','--dsky-fs-glass-y',
  'DISPLAY_PACKAGE_DEPTH_IN','FRAME_DEPTH_IN','const phosphorX = -x * 24.0','const glassX = x * 34.0',
  "elPanel.style.setProperty(\n        'transform'","glassFront.style.setProperty(\n        'transform'",
  'preventDefault(','stopPropagation(','stopImmediatePropagation(','AGCDSKY_APP_STATE','AGCDSKY_CORE_SESSION',
...
for(const forbidden of [
  ...
  'var(--dsky-fs-el-x,0px)','var(--dsky-fs-glass-x,0px)','translate3d(\n      var(--dsky-fs-glass-x',
...
console.log('  restored controls: TILT 0–200% + DEPTH 0–200%, persisted independently');
console.log(`  2004745 viewing thickness remains fixed: ${edgeDepth.toFixed(3)} + ${centerRise.toFixed(3)} = ${viewDepth.toFixed(3)} in -> ${baseGlassPx.toFixed(3)} px at 106-unit width`);