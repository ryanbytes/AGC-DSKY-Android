...
/*
 * Page-recreation semantic smoke for AGC-DSKY Android.
 *
...
if (!Number.isInteger(port) || port < 1 || port > 65535) {
  console.error('usage: node tools/device-recreation-smoke.js <forwarded-devtools-port>');
  process.exit(2);
}
...
async function poll(cdp, label, predicate, timeoutMs = 20000) {
  ...
  const detail = lastError ? `; last evaluation error: ${lastError.message}` : '';
  throw new Error(`${label} timed out; last state: ${JSON.stringify(last)}${detail}`);
}
...
async function main() {
  ...
  try {
    ...
    const tagged = await cdp.evaluate(`(() => {
      ...
      c.__deviceRecreationSentinel = ${JSON.stringify(sentinel)};
      return c.__deviceRecreationSentinel === ${JSON.stringify(sentinel)};
    })()`);
    assert(tagged === true, 'could not tag pre-reload AGC core');

    const persisted = await cdp.evaluate(`(() => ({
      mission: localStorage.getItem('agcMission'),
      runMode: localStorage.getItem('runMode')
    }))()`);
    assert(persisted.mission === 'comanche055' && persisted.runMode === 'agc',
      `pre-reload persistence mismatch: ${JSON.stringify(persisted)}`);

    await cdp.call('Page.reload', { ignoreCache: true });
    await delay(300);

    const after = await poll(cdp, 'AGC page recreation restore', (s) =>
      s.ready
    ...
    assert(after.sentinel === null,
      'page recreation retained the tagged pre-reload core object; a fresh core was expected');
...
main().catch((error) => {
  console.error(`DEVICE PAGE RECREATION SMOKE FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
