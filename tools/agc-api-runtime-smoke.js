...
assert(initCount===1,'API bootstrap did not initialize shell once');assert(api.getCore()===core&&api.getMission()==='comanche055','core-session/mission facade changed');assert(!Object.getOwnPropertyDescriptor(context,'agcCore'),'API smoke must not recreate agcCore global');
...
for(const marker of ['const apiState=window.AGCDSKY_APP_STATE;','const apiCore=window.AGCDSKY_CORE_SESSION;','getCore:()=>apiCore.core','function publicEnterAgc()','function publicEnterClock()','window.AGCDSKY_RUNTIME','enterClock:publicEnterClock','enterAgc:publicEnterAgc','initializeAppShell();'])assert(source.includes(marker),`API runtime missing ${marker}`);
for(const forbidden of ['agcCore','URLSearchParams','localStorage','new AgcCore(','function decodeChannel10(','SNAPSHOT_KEY','.keyPress(','writeIo(0o15','setInterval(','document.addEventListener'])assert(!source.includes(forbidden),`API runtime crossed explicit subsystem/core boundary: ${forbidden}`);
...
console.log('  explicit core-session facade, stable transition identity, lifecycle fallback, dynamic delegation, CLOCK preserve semantics, and shared-state access verified');
