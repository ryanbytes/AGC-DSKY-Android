...
public final class SensorMainActivity extends Activity implements SensorEventListener {
    ...
    private AmbientBrightnessController ambientBrightness;
    ...
    @Override protected void onCreate(Bundle state){
        ...
        ambientBrightness=new AmbientBrightnessController(this);
    ...
    private void configureSensors(){
        sensorManager=(SensorManager)getSystemService(SENSOR_SERVICE);if(sensorManager==null)return;
        attitudeSensor=sensorManager.getDefaultSensor(Sensor.TYPE_GAME_ROTATION_VECTOR);
        if(attitudeSensor!=null)sensorName="game_rotation_vector";else{attitudeSensor=sensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR);if(attitudeSensor!=null)sensorName="rotation_vector";}
    ...
    private void configureWindow(){
        Window window=getWindow();window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON|WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.P){WindowManager.LayoutParams params=window.getAttributes();params.layoutInDisplayCutoutMode=WindowManager.LayoutParams.LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES;window.setAttributes(params);}
        View decor=window.getDecorView();decor.setBackgroundColor(CM_PANEL_COLOR);if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.R)configureModernSystemBars(window);else configureLegacySystemBars(window,decor);
    }
    @SuppressWarnings("deprecation") private static void configureModernSystemBars(Window window){window.setDecorFitsSystemWindows(false);if(Build.VERSION.SDK_INT<Build.VERSION_CODES.VANILLA_ICE_CREAM){window.setStatusBarColor(CM_PANEL_COLOR);window.setNavigationBarColor(CM_PANEL_COLOR);}WindowInsetsController controller=window.getInsetsController();if(controller!=null){controller.hide(WindowInsets.Type.systemBars());controller.setSystemBarsBehavior(WindowInsetsController.BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE);}}
    ...
    private int displayAngleDegrees(){Display display=Build.VERSION.SDK_INT>=Build.VERSION_CODES.R?getDisplay():legacyDefaultDisplay();if(display==null)return 0;switch(display.getRotation()){case Surface.ROTATION_90:return 90;case Surface.ROTATION_180:return 180;case Surface.ROTATION_270:return 270;default:return 0;}}
    @SuppressWarnings("deprecation") private Display legacyDefaultDisplay(){return getWindowManager().getDefaultDisplay();}
    ...
    private boolean isLocalAssetOrigin(String origin){if(origin==null)return false;try{Uri candidate=Uri.parse(origin);int port=candidate.getPort();return "https".equalsIgnoreCase(candidate.getScheme())&&LOCAL_ASSET_ORIGIN.getHost()!=null&&LOCAL_ASSET_ORIGIN.getHost().equalsIgnoreCase(candidate.getHost())&&(port==-1||port==443);}catch(RuntimeException ignored){return false;}}
    ...
    void startDsky(){
        ...
        webView.setWebChromeClient(new WebChromeClient(){
            ...
            @Override public boolean onConsoleMessage(ConsoleMessage message){if(message!=null&&message.messageLevel()==ConsoleMessage.MessageLevel.ERROR){String text=message.message();if(text==null||!text.startsWith("[yaAGC]"))DebugReporter.appendWebError(SensorMainActivity.this,message.sourceId()+":"+message.lineNumber()+"\n"+String.valueOf(text));}return super.onConsoleMessage(message);}
        });
        setContentView(webView);boolean restored=false;if(pendingWebViewState!=null){try{restored=webView.restoreState(pendingWebViewState)!=null;}catch(RuntimeException ignored){}pendingWebViewState=null;}if(!restored)webView.loadUrl(NetClient.ASSET_ORIGIN+NetClient.ASSET_PREFIX+"index.html");pushSensorAvailability();if(ambientBrightness!=null)ambientBrightness.pushStatusDelayed(500L);
    }
    ...
    @Override public void onSensorChanged(SensorEvent event){
        ...
        if(event.sensor==attitudeSensor){if(lastSensorPushNs!=0L&&event.timestamp-lastSensorPushNs<SENSOR_PUSH_INTERVAL_NS)return;lastSensorPushNs=event.timestamp;try{float[] q=new float[4];SensorManager.getQuaternionFromVector(q,event.values);int angle=displayAngleDegrees();String js=String.format(Locale.US,"if(window.AGCDSKY&&AGCDSKY.nativePhoneQuaternion){AGCDSKY.nativePhoneQuaternion(%.9f,%.9f,%.9f,%.9f,%d)}",q[0],q[1],q[2],q[3],angle);webViewHandler.post(()->{if(webView!=null)webView.evaluateJavascript(js,null);});if(event.sensor==magneticAttitudeSensor){pushMagneticQuaternion(q,angle);pushSkyPointing(event.values);}}catch(RuntimeException ignored){}return;}
    ...
    private void pushMagneticQuaternion(float[] q,int displayAngle){if(webView==null||q==null||q.length<4)return;String js=String.format(Locale.US,"if(window.AGCDSKY&&AGCDSKY.nativeMagneticQuaternion){AGCDSKY.nativeMagneticQuaternion(%.9f,%.9f,%.9f,%.9f,%d,%d)}",q[0],q[1],q[2],q[3],displayAngle,magneticAccuracy);webViewHandler.post(()->{if(webView!=null)webView.evaluateJavascript(js,null);});}
    ...
    @Override protected void onResume(){super.onResume();configureWindow();registerSensors();if(ambientBrightness!=null)ambientBrightness.onResume();if(webView!=null){webView.onResume();webView.evaluateJavascript(JS_APP_VISIBLE,null);pushSensorAvailability();if(ambientBrightness!=null)ambientBrightness.pushStatusDelayed(150L);}}
    @Override protected void onPause(){if(ambientBrightness!=null)ambientBrightness.onPause();unregisterSensors();webViewHandler.removeCallbacksAndMessages(null);if(webView!=null){webView.evaluateJavascript(JS_APP_HIDDEN,null);webView.onPause();}super.onPause();}
    ...
    private void destroyWebView(){WebView doomed=webView;webView=null;if(ambientBrightness!=null)ambientBrightness.detach(doomed);WebViewTeardown.destroy(doomed,"DebugBridge","SkyBridge","TimeBridge","BrightnessBridge");}
    @Override protected void onDestroy(){DebugReporter.dismissPendingReport(this);NtpTime.removeListener(ntpListener);unregisterSensors();webViewHandler.removeCallbacksAndMessages(null);pendingWebViewState=null;if(pendingGeoCallback!=null){try{pendingGeoCallback.invoke(pendingGeoOrigin,false,false);}catch(RuntimeException ignored){}}pendingGeoOrigin=null;pendingGeoCallback=null;if(pendingCameraRequest!=null){pendingCameraRequest.deny();pendingCameraRequest=null;}destroyWebView();if(ambientBrightness!=null){ambientBrightness.destroy();ambientBrightness=null;}sensorManager=null;attitudeSensor=null;magneticAttitudeSensor=null;linearAccelerationSensor=null;accelerometerSensor=null;gravitySensor=null;super.onDestroy();}
}
