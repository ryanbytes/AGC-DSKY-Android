...
(() => {
  ...
  if (!fidelityState) throw new Error('Shared application state unavailable');
  const T4_MS = 120;
  ...
  function changedArmatures(prior, target) {
    ...
    for (let bit = 0; bit < 11; bit++) {
      ...
      out.push({ bit, mask, on: !!(target & mask), delay: ARMATURE_SETTLE_MS[bit] });
    }
  ...
  function auxRelayClack(engaging, releasing) {
    ...
    if (!fidelityState.tickSound || count <= 0) return;
    const ctx = ensureAudio();
    if (!ctx) return;
    const play = () => {
  ...
  function currentClockLatchState(commandVerb = fidelityState.verb, commandNoun = fidelityState.noun) {
    ...
    for (const row of CLOCK_ROWS) state[row.relay] = (clockRelayWords[row.relay] ?? 0) & 0o3777;
    return state;
  }
  ...
  decodeChannel11 = function hardwareDecodeChannel11(value) {
    const word = value & 0o77777;
    setAuxRelays({ comp: !!(word & 0o00002), uplink: !!(word & 0o00004), flash: !!(word & 0o00040) }, false);
  ...
  runRelayQueue = function hardwareRunRelayQueue() {
    ...
    const step = () => {
      if (token !== hw.clockToken || fidelityState.mode !== 'clock' || lampTestActive) {
        relayBusy = false;
        return;
      }
      ...
      const prior = Object.prototype.hasOwnProperty.call(hw.latches, relay) ? hw.latches[relay] : (clockRelayWords[relay] ?? 0);
      hw.latches[relay] = prior & 0o3777;
      beginRelayDrive(relay, low11, true);
      clockRelayWords[relay] = low11;
      later(() => {
        if (token !== hw.clockToken || fidelityState.mode !== 'clock' || lampTestActive) return;
  ...
  lampTest = function hardwareLampTest() {
    ...
    scheduleSyntheticFlash();
    const prior = currentClockLatchState(fidelityState.verb, fidelityState.noun);
    ...
    lampTestTimer = setTimeout(() => {
      ...
      setAuxRelays({ uplink: false, temp: false, restart: false, stby: false, flash: false }, true);
      hw.v35DirectOperErr = false;
      hw.v35FlashEnabled = false;
      if (hw.v35FlashTimer) clearTimeout(hw.v35FlashTimer);
      hw.v35FlashTimer = 0;
      document.body.classList.remove('vn-flash-off');
      setAuxRelays({ oprerr: false, keyrel: hw.v35DirectKeyRel }, true);
      ...
      scheduleSyntheticRows(restoreData.state, token, () => {
        ...
        fidelityState.verb = '16';
        fidelityState.noun = '65';
        clockDigits = {
  ...
  setInterval(() => {
    try { if (fidelityState.mode === 'clock' && !lampTestActive && !relayBusy) tick(); } catch (_) {}
  }, 20);
