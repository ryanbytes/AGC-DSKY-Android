...
(() => {
  ...
  const displayButton=document.getElementById('display'),el=document.getElementById('elpanel');
  ...
  function exit(){if(isDream){try{if(window.DreamBridge&&DreamBridge.finishDream)DreamBridge.finishDream()}catch(_){};return}enabled=false;apply();shell.showControls()}
  ...
  if(displayButton)displayButton.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();if(enabled){if(isDream)exit();else{enabled=false;apply();shell.showControls()}}else enter()},true);
  ...
  document.addEventListener('pointerdown',()=>{if(!enabled)return;downAt=performance.now();held=false;clearTimeout(hold);hold=setTimeout(()=>{held=true;exit()},1800)},{passive:true});
  document.addEventListener('pointerup',()=>{if(!enabled){clearTimeout(hold);return}const dt=performance.now()-downAt;clearTimeout(hold);if(!held&&dt<650)toggleTick()},{passive:true});
