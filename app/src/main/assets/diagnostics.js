...
(() => {
  ...
  function update(){
    ...
    if(pwa)h+=row('Web sensor bridge',`${pwa.browser||'unknown'} · orient ${pwa.orientation||'---'} · absolute ${pwa.absoluteOrientation||'---'} · motion ${pwa.motion||'---'}${pwa.sensorBlock?' · '+pwa.sensorBlock:''}`);
