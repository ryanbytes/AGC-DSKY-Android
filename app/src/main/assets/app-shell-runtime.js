...
appState.selectedMission='comanche055';store.set('agcMission','comanche055');
const restoreAgcOnLoad=!dream&&store.get('runMode')!=='clock';
let dreamMode=store.get('dreamMode')||(oldDreamBright?'bright':'dim');
let dim=store.get('dim')==='1',
    tickSound=store.get('audioTickV4')!=='0',
    displayOnly=dream||store.get('displayOnly')==='1',
    controlsTimer=0;

function show(v,n){appState.verb=v;appState.noun=n;set2('verb',v.padStart(2,' '));set2('noun',n.padStart(2,' '))}

function accurateTime(){return Date.now()+(Number(appState.ntpStatus.offsetMs)||0)}
function accurateDate(){return new Date(accurateTime())}
function clockTimeLabel(){return appState.ntpStatus.state==='synced'?'PHONE CLOCK · NTP TIME':appState.ntpStatus.state==='stale'?'PHONE CLOCK · NTP OFFSET STALE':'PHONE CLOCK · ANDROID WALL TIME'}
function updateNtpStatus(value){try{const parsed=typeof value==='string'?JSON.parse(value):value;if(parsed&&typeof parsed==='object'){appState.ntpStatus={...appState.ntpStatus,...parsed};if(appState.mode==='clock')$('mode').textContent=clockTimeLabel()}}catch(_){/* malformed bridge data must not affect the DSKY */}}
...
function missionSpec(){return MISSIONS[appState.selectedMission]}
...
function cycleMission(){appState.selectedMission='comanche055';store.set('agcMission','comanche055');applyMissionButton()}
...
function initializeAppShell(){
  ...
  addEventListener('pagehide',()=>{if(appState.mode==='agc'&&agcCore){agcCore.stop();saveAgcState('page hide')}});
  ...
  loadNativeNtpStatus();applyDim();applyDreamMode();applyDisplayOnly();applyTickSound();applyMissionButton();clearLamps();set2('prog','00');show(appState.verb,appState.noun);syncClockFace();
  setInterval(tick,20);
  setInterval(loadNativeNtpStatus,60000);
  setInterval(()=>{if(appState.mode==='agc'&&agcCore&&agcCore.running&&appVisible&&Date.now()-lastAutosaveAt>15000)saveAgcState('periodic autosave')},5000);
