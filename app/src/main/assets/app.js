...
const MISSIONS=Object.freeze({comanche055:{label:'COMANCHE055',short:'CM C55',rope:'Comanche055.bin'}});
...
let verb='16',noun='65',mode='clock',dim=store.get('dim')==='1',tickSound=store.get('audioTickV4')!=='0',displayOnly=dream||store.get('displayOnly')==='1',controlsTimer=0;
...
function updateNtpStatus(value){try{const parsed=typeof value==='string'?JSON.parse(value):value;if(parsed&&typeof parsed==='object'){ntpStatus={...ntpStatus,...parsed};if(mode==='clock')$('mode').textContent=clockTimeLabel()}}catch(_){}}
function loadNativeNtpStatus(){try{if(window.TimeBridge&&typeof TimeBridge.getStatus==='function')updateNtpStatus(TimeBridge.getStatus())}catch(_){}}
...
function enterClock(status=clockTimeLabel(),preserveAgc=false){
  ...
  agcSuspendedForClock=!!canResume;agcPausedForVisibility=false;mode='clock';rememberRunMode('clock');
...
async function enterAgc(){
  ...
  cancelLampTest();const selected=missionSpec();
  if(agcSuspendedForClock&&agcCore&&agcLoadedMission===selectedMission){
    ...
    if(appVisible){agcCore.start(1);agcPausedForVisibility=false}else agcPausedForVisibility=true;return;
  }
  mode='agc-loading';stopClockQueue();$('agc').textContent='...';$('mode').textContent=`LOADING ${selected.label} · AGC`;resetAgcFace();
  try{
    if(!agcCore||agcLoadedMission!==selectedMission){if(agcCore)agcCore.stop();agcCore=new AgcCore({onChannelUpdate:onAgcChannel,onError:agcFailure});await agcCore.load({wasmUrl:'yaAGC.wasm',ropeUrl:selected.rope});agcLoadedMission=selectedMission}
    else{agcCore.reset();agcCore.configureInputMasks()}
    const restored=restoreSavedAgcState();mode='agc';agcSuspendedForClock=false;rememberRunMode('agc');$('agc').textContent='AGC MODE';$('mode').textContent=`${selected.label} · ${agcCore.version()}${restored?' · STATE RESTORED':''}`;
    if(restored)renderAgcSnapshot();if(appVisible){agcCore.start(1);agcPausedForVisibility=false}else agcPausedForVisibility=true;
  }catch(error){agcFailure(error)}
}
function agcFailure(error){agcSuspendedForClock=false;console.error('AGC core stopped',error);enterClock('AGC ERROR · PHONE CLOCK',false)}
function cycleMission(){selectedMission='comanche055';store.set('agcMission','comanche055');applyMissionButton()}
function setAppVisible(visible){appVisible=!!visible;if(mode!=='agc'||!agcCore)return;if(!appVisible){if(agcCore.running){agcCore.stop();agcPausedForVisibility=true}saveAgcState('app background');return}if(agcPausedForVisibility){agcPausedForVisibility=false;agcCore.start(1)}}
...
let holdTimer=0;
document.addEventListener('pointerdown',e=>{if(dream)return;if(displayOnly){holdTimer=setTimeout(()=>{displayOnly=false;applyDisplayOnly();showControls()},1800);return}if(e.target.closest('[data-key],.app-controls'))return;holdTimer=setTimeout(showControls,620)},{passive:true});
document.addEventListener('pointerup',()=>clearTimeout(holdTimer),{passive:true});document.addEventListener('pointercancel',()=>clearTimeout(holdTimer),{passive:true});document.addEventListener('visibilitychange',()=>setAppVisible(!document.hidden));
addEventListener('pagehide',()=>{if(mode==='agc'&&agcCore){agcCore.stop();saveAgcState('page hide')}});
$('dim').addEventListener('click',()=>{dim=!dim;applyDim();showControls()});$('dreambright').addEventListener('click',()=>{cycleDreamMode();showControls()});$('sound').addEventListener('click',()=>{ensureAudio();tickSound=!tickSound;applyTickSound();if(tickSound)playRelayBurst(1);showControls()});$('display').addEventListener('click',()=>{displayOnly=true;applyDisplayOnly()});$('agc').addEventListener('click',()=>{enterAgc();showControls()});$('clock').addEventListener('click',()=>{enterClock(clockTimeLabel(),true);showControls()});document.addEventListener('pointerdown',()=>{if(tickSound)ensureAudio()},{passive:true});
...
if(!dream&&!restoreAgcOnLoad)rememberRunMode('clock');if(restoreAgcOnLoad)setTimeout(()=>enterAgc(),0);
if(dream){updateDreamEnvironment();setInterval(updateDreamEnvironment,15000);const pos=[[0,0],[3,-2],[-3,2],[2,3],[-2,-3],[1,-1]],dsky=$('dsky');let i=0;setInterval(()=>{const p=pos[i++%pos.length];dsky.style.setProperty('--drift-x',p[0]+'px');dsky.style.setProperty('--drift-y',p[1]+'px')},60000)}
