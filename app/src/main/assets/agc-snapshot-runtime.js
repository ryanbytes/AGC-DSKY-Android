...
function saveAgcState(reason='manual'){
  ...
  if(!agcCore||agcLoadedMission!==snapshotState.selectedMission||typeof agcCore.exportSnapshot!=='function')return false;
  try{
    const timestamp=Date.now(),payload={schema:1,mission:snapshotState.selectedMission,coreVersion:agcCore.version(),timestamp,reason,ui:snapshotUiState(),core:agcCore.exportSnapshot()};
...
function verifySnapshotRoundTrip(){
  if(!agcCore||agcLoadedMission!==snapshotState.selectedMission||typeof agcCore.exportSnapshot!=='function'||typeof agcCore.importSnapshot!=='function')return {ok:false,error:'AGC core not ready'};
...
function scheduleAgcAutosave(reason='activity'){
  if(snapshotState.mode!=='agc'||!agcCore)return;
  clearTimeout(autosaveTimer);autosaveTimer=setTimeout(()=>{autosaveTimer=0;if(snapshotState.mode==='agc'&&agcCore&&agcCore.running)saveAgcState('autosave: '+reason)},1800);
}
