...
cp "$PWA/static/pwa-network-time.js" "$DEST/pwa-network-time.js"
...
if any(marker in text for marker in ('manifest.webmanifest', 'pwa-sensor-parity.js', 'pwa-auto-dim.js', 'pwa-clock-guard.js', 'pwa-network-time.js', 'pwa-print-bridge.js', 'pwa-bootstrap.js', 'analytics.js')):
